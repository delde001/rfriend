...
- model1_name:
  ...
  `NULL` (default), the function uses `deparse(substitute(model1))`.

- model2_name:

  Optional character string. A custom name for model2 in the output. If
  `NULL` (default), the function uses `deparse(substitute(model2))`.
...
- model2_name:
  ...
  The name of the second model (always the more complex model when
  nested).
...
- swapped:
  ...
  Logical indicating whether the model order was swapped to ensure
  model1 is the simpler model.
...
When nested models are detected or specified, model1 is always treated
as the simpler model (fewer parameters). If the user passes the complex
model first, the function automatically swaps them and issues a message.
...
#> 
#>       - model1 ('simple', df=2) is the simpler model
#> 
#>       - model2 ('complex', df=4) is the more complex model.
...
print(comparison)
#> Comparison of two nested models:
#> Model1: Weight only (glm)
#> Model2: Weight + Horsepower (glm)
#> 
#>           Metric Model1 Model2 Difference
#>              AIC 23.176 16.059     -7.117
#>              BIC 26.108 20.456     -5.651
